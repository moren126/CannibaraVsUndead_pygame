...
def drawselectedButton(self, msg, fontSize, x, y, w, h):
    ...
    fontButton = pygame.font.Font(self.FONTNAMEITALIC, fontSize)    
    textSurfButton = fontButton.render(msg, True, WHITE)
    textRectButton = textSurfButton.get_rect()
    textRectButton.center = ( (x + (w/2)), (y + (h/2)) )
    self.displaySurface.blit(textSurfButton, textRectButton)  
    
def caption(obj, msg, fontSize, x, y, w, h, centerX, centerY, isItalic, color = (255, 255, 255)):

    if x != 0 or y != 0:
        pygame.draw.rect(obj.displaySurface, RED, (x, y, w, h))
    
    if isItalic:
        font = obj.FONTNAMEITALIC
    else:
        font = obj.FONTNAME
  
    fontButton = pygame.font.Font(font, fontSize)  
    textSurfButton = fontButton.render(msg, True, color)
    textRectButton = textSurfButton.get_rect()
    textRectButton.center = (centerX, centerY)
    obj.displaySurface.blit(textSurfButton, textRectButton)      